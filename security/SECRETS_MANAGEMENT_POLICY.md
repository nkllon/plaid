...
- `louspringer/chatbot-llm` local testing guidance explicitly states that `.env` files must not be committed and that 1Password is used for secrets.
...
- Secrets must be rotated after suspected exposure, personnel change, or major integration change.
- Long-lived credentials must be replaced with managed identity or short-lived credentials where supported.

## Rotation expectations

- High-risk integration secrets must be rotated at least annually, and sooner when supported by the provider or incident response requirements.
- Emergency rotation must be initiated immediately upon suspected exposure.

- GitHub Actions workflows that require secrets must use repository or environment secrets rather than storing sensitive values in workflow files.

## Validation

- Secret values must be tested in non-production environments before production rollout.
- Repository scanning, manual review, and GitHub secret scanning features must be used to reduce accidental secret exposure.
